...
# NOTE: st.set_page_config is intentionally NOT called here. This module runs as a
# page under st.navigation; the navigation shell (app.py) owns page config.
...
if st.session_state.graph:
    ...
    with tab_overview:
        ...
        if st.session_state.response and st.session_state.response.metadata.get('api_timings'):
            ...
            with col2:
                with_data = sum(1 for t in api_timings if t['success'])
                answered = sum(1 for t in api_timings if t.get('returned_ok'))
                no_data = answered - with_data
                st.metric(
                    "APIs with data", f"{with_data}/{len(api_timings)}", border=True,
                    help=f"{answered} answered with HTTP 200 ({no_data} returned no matching edges); "
                         "the rest were rejected, down, or timed out.",
                )
            with col3:
                ...
                api_df = pd.DataFrame([
                    {
                        "API": t['api_name'],
                        "Status": (
                            "Success" if t['success']
                            else "No data" if t.get('error_type') == "no_edges"
                            else (t.get('error_type') or 'Failed').replace('_', ' ').title()
                        ),
                        "Edges": t['edge_count'],
